...
export const formatSurahRange = (range: SurahRange): string => {
  return `${range.surah} (${range.fromVerse} - ${range.toVerse})`;
};

export const formatSurahRanges = (ranges: SurahRange[]): string[] => {
  return ranges.map(formatSurahRange);
};