...
import * as lessonSvc from "@/src/utils/services/lesson.service";
...
export const LessonsProvider = ({ children }: LessonsProviderProps) => {
  ...
  const getLessonById = useCallback(async (lessonId: string) => {
    try {
      ...
      setError("خطأ في جلب بيانات الحلقة");
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const completeLesson = useCallback(async (lessonId: string) => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await lessonSvc.completeLesson(lessonId); // Trigger refresh
      setLessonsRefreshKey((k) => k + 1);
      return data;
    } catch (error) {
      setError("خطأ في إتمام الحلقة");
      throw error;
    } finally {
      setIsLoading(false);
    }
